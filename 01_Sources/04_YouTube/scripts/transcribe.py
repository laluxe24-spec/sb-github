#!/usr/bin/env python3
"""YouTube動画のURLを渡すと、音声のみダウンロード→文字起こしを行い、
data/@チャンネル名/transcripts/ に文字起こしMarkdownを保存する(03_TikTok/data/@アカウント名/ と同じ構成)。

使い方:
    python3 transcribe.py <YouTubeのURL> [<URL2> ...]
...
def save_transcript(account_dir: Path, info: dict, text: str) -> Path:
    ...
    frontmatter = "\n".join(
        [
            ...
            f"title: {(info.get('title') or '').strip()[:80]}",
...
def update_manifest(account_dir: Path, info: dict) -> None:
    ...
    entry = {
        ...
        "title": (info.get("title") or "").strip()[:80],
...
def process(url: str) -> None:
    ...
    print("[2/3] 文字起こし中...(YouTubeは動画が長いので数分〜数十分かかることがある)")
...
def main() -> None:
    if len(sys.argv) < 2:
        print("使い方: python3 transcribe.py <YouTubeのURL> [<URL2> ...]")
