...
参考画像(コマ抜き)は`02_Sources/03_TikTok/data/@ichiroomvlog/frames/`に保存済み。

1. **俯瞰(トップダウン)ショットは真上ではなく少し斜め**: 完全に真上(90度)ではなく、対象物のテーブルが画面に対して斜め(15〜30度くらい)傾いて写る角度で撮ってる。三脚で完全固定するより、スマホを片手で持って体ごと対象物に覆いかぶさるように構えると同じ雰囲気になる。手ブレが多少残る方が「作業感・臨場感」が出て自然に見える。
   → 参考画像: `02_俯瞰ショット_斜め.jpg`
2. **手元作業は近距離(20〜30cm目安)**: 手とパーツが画面いっぱいに映るくらいスマホを対象に近づけて撮ってる。広角カメラのまま近づくと端が歪みやすいので、歪みが気になる場合はスマホの「1倍(標準)」カメラを使う方が安全。
3. **商品ショット(完成品)は目線の高さ・水平固定**: 冒頭とラストの完成品カットは、カメラを商品と同じ高さに水平に構えて正面から撮ってる。三脚やスマホスタンドで固定すると再現しやすい。商品より暗い色(このケースは黒いフレーム)を使い、背景(白い壁)とのコントラストで商品を浮き立たせている。
   → 参考画像: `01_商品ショット_正面.jpg`
4. **背景に「生活感のある小物」を1〜2個仕込む**: モノトーンの背景の中に、ラグ(装飾マット)やアクセサリーラックなど、色や質感に主張のある小物を画角の隅にわざと入れてる。真っ白・真っ黒な無地背景ではなく、部屋の一部として見せることで「実際に使ってる感」を出す工夫(`01_商品ショット_正面.jpg`の背景にも同じ工夫が写ってる)。
5. **ブルーのネオンライン照明はアクセント用**: 商品自体を照らす主照明ではなく、背景の奥に置いて雰囲気作りだけに使ってる(USB給電のLEDネオンチューブなど、安価なもので再現できる)。主照明は自然光か部屋の照明で十分そうで、特別な機材は使ってなさそう。
6. **速いパンは「体ごと半歩振る」感覚で**: 場面転換のパンは、腕だけでスマホを振るというより、体ごと軽く回転しながら振ってるようなブレ方をしてる。編集で速度を上げてる可能性もあるが、実際に素早く振ってから編集でさらにテンポを詰める、くらいの気持ちで撮ると近づけやすい。
   → 参考画像: `03_パン_モーションブラー.jpg`
